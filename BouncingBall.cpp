...
const float GRAVITY_MERCURY = 3.7f * 1000;
const float GRAVITY_VENUS = 8.87f * 1000;
const float GRAVITY_EARTH = 9.81f * 1000;
const float GRAVITY_MARS = 3.71f * 1000;
const float GRAVITY_JUPITER = 24.79f * 1000;
const float GRAVITY_SATURN = 10.44f * 1000;
const float GRAVITY_URANUS = 8.69f * 1000;
const float GRAVITY_NEPTUNE = 11.15f * 1000;
const float GRAVITY_PLUTO = 0.62f * 1000;
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // If the object hits the ground, reverse its vertical velocity (perfect bounce)
        if (position[1] >= groundLevel) {
            position[1] = groundLevel;
            velocity[1] *= -(velocity[1]); // Reverse the vertical velocity (no loss of energy)
            collisionCount++;
        }
